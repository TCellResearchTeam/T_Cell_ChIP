...
bam_files = glob.glob("%s/*_srt_dupr_flb.bam"%bam_dir)

exist_srr_list = []
for file in fastq_files + bam_files:
    file_size = os.stat(file).st_size
    if file_size > 1048576:
        exist_srr_list.append(file.split("/")[-1].split("_")[0])
exist_srr_list = list(set(exist_srr_list))
